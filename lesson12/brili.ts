...
let traceStartLabel: string = "trace_start";
let traceEndLabel: string = "trace_end";
 ...
 */
function evalInstr(instr: bril.Instruction, state: State, i: number): Action {
  state.traceInstr = state.trace;
  ...
  switch (instr.op) {
    ...
    case "print": {
      if (state.trace) {
        console.error(
          JSON.stringify({ op: "commit", index: i, labels: [traceEndLabel] }),
        );
        console.error(JSON.stringify({ op: "jmp", labels: [traceEndLabel] }));
        state.trace = false;
        state.traceInstr = false;
      }
      let args = instr.args || [];
          ...
          if (state.trace) {
            console.error(
              JSON.stringify({
                op: "commit",
                index: i,
                labels: [traceEndLabel],
              }),
            );
            console.error(
              JSON.stringify({ op: "jmp", labels: [traceEndLabel] }),
            );
            state.trace = false;
            state.traceInstr = false;
          }
...
function evalProg(prog: bril.Program) {
  ...
  // Silly argument parsing to find the `-t` flag.
  let tracing = false;
  pidx = args.indexOf("-t");
  if (pidx > -1) {
    tracing = true;
    args.splice(pidx, 1);
    console.error(
      JSON.stringify({ op: "speculate", index: 0, labels: [traceStartLabel] }),
    );
  }
